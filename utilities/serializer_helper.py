...
class JalaliDateField(DateField):
    ...
    def to_internal_value(self, value):
        try:
            jy, jm, jd = str(value).split('/')
            y, m, d = jalali_to_gregorian(int(jy), int(jm), int(jd))
            return super().to_internal_value(datetime.date(y, m, d))
        except:
            raise ValidationError('مقدار ارسال شده معتبر نمیباشد!')

    def to_representation(self, value):
        jy, jm, jd = str(super().to_representation(value)).split('-')
        jy, jm, jd = gregorian_to_jalali(int(jy), int(jm), int(jd))
        return f"{jy}/{jm}/{jd}"
